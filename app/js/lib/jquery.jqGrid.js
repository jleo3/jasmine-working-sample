...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
            ...
            $('.ui-pg-selbox',"#"+pgcnt).dropkick({
                change: function (value, label) {
                    ts.p.page = Math.round(ts.p.rowNum*(ts.p.page-1)/value-0.5)+1;
                    ts.p.rowNum = value;
                    if(ts.p.pager) { $('.ui-pg-selbox',ts.p.pager).val(value); }
                    if(ts.p.toppager) { $('.ui-pg-selbox',ts.p.toppager).val(value); }
                    if(!clearVals('records')) { return false; }
                    populate();
                    return false;
                }
...
;(function($){
/*
 * jqGrid extension for constructing Grid Data from postdeploy file
 * Tony Tomov tony@trirand.com
 * http://trirand.com/blog/ 
 * Dual licensed under the MIT and GPL licenses:
 * http://www.opensource.org/licenses/mit-license.php
